...
datfile = r'C:\Users\mugdhapolimera\github\SDSS_spectra\resolvecatalog.dat'
data = readsav(datfile)
cz = pd.DataFrame.from_dict(data['vhel'])
cz.index = [name.decode('utf-8') for name in data['name']]
...
heii = resolve.NAME[(resolve.Flux_HeII_4685 > 5*resolve.Flux_HeII_4685_Err) & 
                    (resolve.Flux_HeII_4685_Err > 0)]
#['rf0013', 'rf0372', 'rs0463', 'rs1103', 'rs1111', 'rs1214']
...
for gal in heii:#datalist.index:
    if gal in resolve.index:
        ...
        new_spec = Spectrum1D(spectral_axis = new_wave* u.AA , flux = new_flux *u.Unit('erg cm-2 s-1 AA-1')) # doctest: +REMOTE_DATA
        
        heii_wave = new_wave[(new_wave > 4665) & (new_wave < 4705)]
        heii_flux = new_flux[(new_wave > 4665) & (new_wave < 4705)]
        heii_spec = Spectrum1D(spectral_axis = heii_wave* u.AA , flux = heii_flux *u.Unit('erg cm-2 s-1 AA-1')) # doctest: +REMOTE_DATA
        ...
        peak = new_wave[(new_wave<upper) & (new_wave>lower)][np.argmax(new_flux[(new_wave<upper) & (new_wave>lower)])]
        print(peak, equivalent_width(new_spec, regions=SpectralRegion(lower*u.AA, upper*u.AA)))
