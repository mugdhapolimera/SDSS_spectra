...
#t1 = fits.open('coords/resolve_SDSS_one.fits')
#t2 = fits.open('coords/resolve_SDSS_two.fits')
...
#hdu.writeto('RESOLVE_SDSS_raw.fits', clobber= True)


#open the file
hdulist = fits.open('RESOLVE_SDSS_raw.fits')
#dwarf
#hdulist = fits.open('coords/resolve_SDSS_dwarf.fits')
#extract the data
...
#tbhdu.writeto('RESOLVE_SDSS_dext_dwarf.fits', clobber=True)
#####################################################################
...
active_stars =np.loadtxt('michaelgals.txt', dtype = np.str)
...
active, = ax.plot(activex[no_zero_active],activey[no_zero_active],"ro", markersize = 8)
ax.set_xlim(-3,2)
ax.set_ylim(-3,4)
ax.set_xlabel(r"$\rm \log([NII]/H\alpha)$", fontsize = 22)
ax.set_ylabel(r"$\rm \log([OIII]/H\beta)$", fontsize = 22)
ax.set_title("SDSS RESOLVE BPT Diagnostic Plot", fontsize = 20)
...
active, = ax_heii.plot(activex_heii[no_zero_active_heii],activey_heii[no_zero_active_heii],"ro", markersize = 8)
