...
odonnell_dat = np.loadtxt('smcbar_extinction.txt', dtype = "float")
#SMC lambda - using Milky Way Extinction
mw_lam = odonnell_dat[:,2]
#factor to relate each mw lam and color excess
mw_A_overEBV = odonnell_dat[:,3]
k_Hbeta = mw_A_overEBV[620]
k_Halpha = mw_A_overEBV[1187]
...
EBV_excess= (2.5/(k_Hbeta - k_Halpha))*(np.log10(balm_dec_obs) - np.log10(balm_dec_exp)) 
print EBV_excess[0]
#speed of light
c_kms = 3.0*10**5.0
...
for i in np.arange(1):#len(galname)):
...
#tbhdu.writeto('RESOLVE_SDSS_full_smcdext.fits', clobber=True)
#####################################################################
